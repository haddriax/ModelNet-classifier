...
def plot_training_efficiency_per_model(runs: list[dict], output_dir: Path, plt) -> None:
    """Ablation efficiency: one figure per model, each bar = one configuration.

    Each figure has a dual axis: epochs trained (bars) and wall-clock time (line).
    Bars are labelled "{sampling_method}_{n_points}pts_bs{batch_size}", sorted by
    n_points → sampling_method → batch_size so configs are easy to compare.

    Filenames: ``ablation_training_efficiency_{ModelName}.png``
    """
    import numpy as np
    ...
    for model_name in models:
        model_runs = sorted(
            [r for r in runs if r["config"]["model"] == model_name],
            key=lambda r: (r["config"]["n_points"], r["config"]["sampling_method"], r["config"]["batch_size"]),
        )
        if not model_runs:
            continue

        labels = [
            f"{r['config']['sampling_method']}_{r['config']['n_points']}pts_bs{r['config']['batch_size']}"
            for r in model_runs
        ]
        epochs_list = [r["metrics"]["epochs_trained"] for r in model_runs]
        time_minutes = [r["metrics"]["total_training_time_seconds"] / 60 for r in model_runs]
        color = model_colors.get(model_name, "#999999")

        x = np.arange(len(model_runs))
        fig, ax1 = plt.subplots(figsize=(max(8, len(model_runs) * 1.4), 6))

        bars = ax1.bar(x, epochs_list, color=color, alpha=0.8, label="Epochs trained")
        ...
        ax2.plot(x, time_minutes, color="#e74c3c", marker="o", linewidth=2,
                 markersize=7, label="Training time (min)")
        ax2.set_ylabel("Training time (min)")
        ax2.set_ylim(0, max(time_minutes) * 1.3)

        for xi, tm in zip(x, time_minutes):
            ax2.text(
                xi,
                tm + max(time_minutes) * 0.04,
                f"{tm:.1f}m",
        ...
        ax1.set_title(f"Training Efficiency — {model_name}")
        ax1.grid(axis="y", alpha=0.3)
        plt.tight_layout()
        plt.savefig(output_dir / f"ablation_training_efficiency_{model_name}.png", dpi=150)
        plt.close()


def plot_training_efficiency_per_npoints(runs: list[dict], output_dir: Path, plt) -> None:
    """Ablation efficiency: one figure per n_points value, one bar per model.

    Each bar shows the average training time (in minutes) across all sampling
    methods and batch sizes for that model / n_points combination.

    Filenames: ``ablation_training_efficiency_npts{N}.png``
    """
    import numpy as np

    model_colors = _get_model_colors()
    models = sorted(set(r["config"]["model"] for r in runs))
    all_npoints = sorted(set(r["config"]["n_points"] for r in runs))

    for n_pts in all_npoints:
        npts_runs = [r for r in runs if r["config"]["n_points"] == n_pts]

        mean_times: list[float] = []
        for model_name in models:
            model_runs = [r for r in npts_runs if r["config"]["model"] == model_name]
            if model_runs:
                avg = sum(r["metrics"]["total_training_time_seconds"] for r in model_runs) / len(model_runs) / 60
            else:
                avg = 0.0
        ...
        bars = ax.bar(x, mean_times, color=colors, alpha=0.85)
        ax.set_ylabel("Training time (min)")
        ax.set_xticks(x)
        ax.set_xticklabels(models, rotation=15, ha="right")
        ax.set_ylim(0, max(mean_times) * 1.25 if max(mean_times) > 0 else 1)
        ax.grid(axis="y", alpha=0.3)

        for bar, tm in zip(bars, mean_times):
            if tm > 0:
                ax.text(
                    bar.get_x() + bar.get_width() / 2,
        ...
        ax.set_title(f"Training Time — {n_pts} Points (avg over all configs)")
        plt.tight_layout()
        plt.savefig(output_dir / f"ablation_training_efficiency_npts{n_pts}.png", dpi=150)
        plt.close()
